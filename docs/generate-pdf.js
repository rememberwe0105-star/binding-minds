...
const inputFile = process.argv[2] || 'backend_api_requirements.md';
const mdContent = fs.readFileSync(path.join(__dirname, inputFile), 'utf-8');
...
const title = mdContent.match(/^# (.+)$/m)?.[1] || 'DearGiver Document';
...
<style>
  ...
  body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Helvetica, Arial, sans-serif; font-size: 11px; line-height: 1.6; color: #1a1a1a; max-width: 800px; margin: 0 auto; padding: 20px; }
...
const outputBase = inputFile.replace('.md', '');
fs.writeFileSync(path.join(__dirname, `${outputBase}.html`), fullHtml);
console.log(`HTML: docs/${outputBase}.html`);
